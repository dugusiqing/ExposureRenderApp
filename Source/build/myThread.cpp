...
// Render thread
myQRenderThread* gpMyRenderThread = NULL;
myQFrameBuffer gMyFrameBuffer;

mutex gMySceneMutex;

int gMyCurrentDeviceID = 0;
...
myQRenderThread::myQRenderThread(const std::string& FileName) :
	//QThread(pParent),
	m_FileName(FileName),
	m_pRenderImage(NULL),
...
myQRenderThread::myQRenderThread(const myQRenderThread& Other)
{
	*this = Other;
}

myQRenderThread::~myQRenderThread(void)
{
...
void myQRenderThread::run()
{
	if (!SetCudaDevice(gMyCurrentDeviceID))
		return;
	...
	gStatus.SetLoadPreset(QFileInfo(m_FileName.c_str()).baseName());
	...
	try
	{
		while (!m_Abort)
		{
			...
			if (std::distance(m_SaveFrames.begin(),find(m_SaveFrames.begin(), m_SaveFrames.end(), SceneCopy.GetNoIterations())) > 0)
			{
				const std::string ImageFilePath = /*QApplication::applicationDirPath().toStdString() +*/ "./Output/" + m_SaveBaseName + "_" + to_string(SceneCopy.GetNoIterations()) + ".png";

				SaveImage((unsigned char*)m_pRenderImage, SceneCopy.m_Camera.m_Film.m_Resolution.GetResX(), SceneCopy.m_Camera.m_Film.m_Resolution.GetResY(), QString::fromStdString(ImageFilePath));
			}
		
			gStatus.SetPostRenderFrame();
		}
	}
	catch (std::string pMessage)
	{
		Log(QString::fromStdString(pMessage + (", rendering will be aborted")));
		free(m_pRenderImage);
		m_pRenderImage = NULL;
...
bool myQRenderThread::Load(std::string& FileName)
{
	m_FileName = FileName;
	...
	if (!FileInfo.exists())
	{
		Log(QString(FileInfo.filePath().replace("//", "/")).toLatin1() + "  does not exist!", QLogger::Critical);
		return false;
	}

	Log(QString("Loading " + FileInfo.fileName()).toLatin1());

	// Exit if the reader can't read the file
	if (!MetaImageReader->CanReadFile(m_FileName.c_str()))
	{
		Log(QString("Meta image reader can't read file " + FileInfo.fileName()).toLatin1(), QLogger::Critical);
		return false;
	}

	MetaImageReader->SetFileName(m_FileName.c_str());
	...
	gStatus.SetStatisticChanged("Volume", "File", FileInfo.fileName(), "");
...
void myQRenderThread::OnUpdateTransferFunction(void)
{
...
void myQRenderThread::OnUpdateCamera(void)
{
	QMutexLocker Locker(&gSceneMutex);
...
void myQRenderThread::OnUpdateLighting(void)
{
	QMutexLocker Locker(&gSceneMutex);
...
void myQRenderThread::OnRenderPause(const bool& Pause)
{
	m_Pause = Pause;
}

CColorRgbLdr* myQRenderThread::GetRenderImage(void) const
{
	return m_pRenderImage;
}

void myQRenderThread(std::string& FileName)
{
	// Create new render thread
	gpMyRenderThread = new myQRenderThread(FileName);

	// Load the volume
	if (!gpMyRenderThread->Load(FileName))
		return;

	// Start the render thread
	gpMyRenderThread->run();
}
...
void myKillRenderThread(void)
{
	...
	gpMyRenderThread->Close();

	// Wait for thread to end
	//gpMyRenderThread->wait();
	gpMyRenderThread->join();
	// Remove the render thread
	delete gpMyRenderThread;
	gpMyRenderThread = NULL;
}